...
public:
    ...
    int imgWidth = 0;
    double fov_h = 0; //in degree
    double fov_v = 0; //in degree
    ...
    FisheyeUndist(const std::string & camera_config_file, int _id, double _fov_h, double _fov_v, bool _enable_cuda = true, int imgWidth = 600):
    imgWidth(imgWidth), fov_h(_fov_h), fov_v(_fov_v), cameraRotation(0, 0, 0), enable_cuda(_enable_cuda), cam_id(_id) {
        ...
        undistMaps = generateAllUndistMap(cam, cameraRotation, imgWidth, fov_h, fov_v);
...
#pragma omp parallel for num_threads(10)
    ...
    std::vector<std::pair<cv::Mat, cv::Mat>> generateAllUndistMap(camodocal::CameraPtr p_cam,
                                          ...
                                          const double &fov_h, //degree
					  const double &fov_v //degree
    ) {
        // ROS_INFO("Generating undistortion maps:");
        double sideVerticalFOV = (fov_v - 180) * DEG_TO_RAD;
        if (sideVerticalFOV < 0)
            sideVerticalFOV = 0;
	double sideHorizontalFOV = (fov_h - 180) * DEG_TO_RAD;
        if (sideHorizontalFOV < 0)
            sideHorizontalFOV = 0;
        double centerFOV = fov_h * DEG_TO_RAD - sideHorizontalFOV * 2;
